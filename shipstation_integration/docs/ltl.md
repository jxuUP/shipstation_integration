...
LTL starts from ERPNext's Shipment document with **Freight Type** set to `LTL` and a **Preferred Carrier** selected. Pack the shipment (assign SDN lines to parcels), **submit** the Shipment, then click **Get LTL Quotes** to request contracted rates from the carrier. Each parcel is rated at the **Parcel Weight** typed while packing (the weight the pallet label and bill of lading print); a parcel with no typed weight is weighed from its Delivery Note items or Item masters. The NMFC class comes from that weight and the parcel dimensions. Saved offers show up behind **Accept Offer** on the Shipment: **Accept and Book Pickup** submits the quotation and books the carrier in one go (the booking, with carrier, PRO, pickup number and cost, then sits at the top of the form), **Accept Only** holds the offer and its cost for **Schedule LTL Pickup** later. **Cancel Booking** (or **Cancel Offer**) tells the carrier, cancels the quotation and puts the Shipment back to Submitted so it can be quoted again. If the carrier supports spot quotes a checkbox appears to request them instead. A progress dialog counts the offers in as the carriers answer (Banyan rates in the background and is polled every few seconds; carriers that answer in one round trip fill the list at once), then lists them to pick from.
