...
class OdflLTL(BaseLTL):
	...
	def odfl_address(info: dict, account: str = "") -> dict:
		...
		block = {
			...
			"stateProvince": OdflLTL.odfl_state(addr["state_province"]),
	...
	# US/CA region names -> 2-letter codes; ODFL rejects anything longer than 2 chars.
	STATE_NAME_TO_CODE = {
	...
	@staticmethod
	def odfl_state(value: str) -> str:
		raw = (value or "").strip().rstrip(".").strip()
		if len(raw) <= 2:
			return raw.upper()
		return OdflLTL.STATE_NAME_TO_CODE.get(raw.title(), raw)
	...
	# NMFC density (lb/cubic ft) -> freight class breakpoints
	ODFL_DENSITY_TO_CLASS = [
		(50.0, 50), (35.0, 55), (30.0, 60), (22.5, 65), (15.0, 70), (13.5, 77.5),
		(12.0, 85), (10.5, 92.5), (9.0, 100), (8.0, 110), (7.0, 125), (6.0, 150),
		(5.0, 175), (4.0, 200), (3.0, 250), (2.0, 300), (1.0, 400),
	]

	SDN_WEIGHT_UOM_TO_LB = {
		"Pound": 1.0, "Lb": 1.0, "Lbs": 1.0,
		"Kg": 2.20462, "Kilogram": 2.20462,
		"Gram": 0.00220462, "Ounce": 1 / 16,
	}

	@classmethod
	def freight_class_from_density(cls, density: float) -> float:
		for threshold, fclass in cls.ODFL_DENSITY_TO_CLASS:
	...
	def apply_manual_parcel_weights(self, doc, packages: list[dict]) -> list[dict]:
		...
		for row in doc.shipment_delivery_note or []:
			if not row.parcel_number or not flt(row.parcel_weight):
				continue
			mult = self.SDN_WEIGHT_UOM_TO_LB.get((row.parcel_weight_uom or "Pound").title(), 1.0)
			num = int(row.parcel_number)
			by_parcel[num] = max(by_parcel.get(num, 0.0), flt(row.parcel_weight) * mult)
		parcel_nums = sorted(
			{int(r.parcel_number) for r in (doc.shipment_delivery_note or []) if r.parcel_number}
		)
		if len(parcel_nums) != len(packages):
			return packages
		for num, pkg in zip(parcel_nums, packages):
			manual_lb = by_parcel.get(num, 0.0)
			computed_lb = self.package_weight_to_pounds(pkg.get("weight") or {})
			if manual_lb <= computed_lb:
				continue
			pkg["weight"] = {"value": manual_lb, "unit": "pounds"}
			dims = pkg.get("dimensions") or {}
			try:
				length_in = self.package_dimension_to_inches(dims.get("length"), dims.get("unit"))
				width_in = self.package_dimension_to_inches(dims.get("width"), dims.get("unit"))
				height_in = self.package_dimension_to_inches(dims.get("height"), dims.get("unit"))
				cubic_ft = (length_in * width_in * height_in) / 1728.0
				if cubic_ft > 0:
					pkg["freight_class"] = self.freight_class_from_density(manual_lb / cubic_ft)
			except Exception:
				pass
		return packages
	...
	def build_rate_soap(self, doc: Shipment, fc) -> str:
		...
		packages = self.apply_manual_parcel_weights(doc, ltl.build_packages_from_sdn(doc))
		freight_items_xml = "\n".join(
		...
		return RATE_SOAP_ENVELOPE.format(
			...
			origin_state=self.xml_escape(self.odfl_state(o["state_province"])),
			...
			dest_state=self.xml_escape(self.odfl_state(d["state_province"])),
	...
	def parse_rate_response(xml_text: str) -> dict:
		...
		for candidate in (
			...
			try:
				if candidate and float(candidate):
					total = candidate
					break
			except (TypeError, ValueError):
				continue
		# netFreightCharge already includes fuel and accessorials on undiscounted lanes
		if not total:
			total = net
		if not total:
			try:
				total = str(float(gross or 0) + float(fuel or 0) + float(accessorial_total or 0))
			except (TypeError, ValueError):
				total = gross or ""
